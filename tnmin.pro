...
;  be modified; and parameters can be tiedg error or warning message
;  is returned.
;
...
;
;              PRO ITERPROC, MYFUNCT, p, iter, fnorm,
;              FUNCTARGS=fcnargs, $
...
;
;             Default value:  all parameters are free and
;             unconstrained.
;
...
; IN WHICH A LOWER POINT IS TO BE FOUND AND FROM THIS GETPTC COMPUTES
; A
...
;      SUBROUTINE TNBC (IERROR, N, X, F, G, W, LW, SFUN, LOW, UP,
;      IPIVOT)
...
; PP. 770-778).  THIS ALGORITHM FINDS A LOCAL MINIMUM OF F(X).  IT
; DOES
; NOT ASSUME THAT THE FUNCTION F IS CONVEX (AND SO CANNOT GUARANTEE A
; GLOBAL SOLUTION), BUT DOES ASSUME THAT THE FUNCTION IS BOUNDED
; BELOW.
...
;           ESTIMATE OF THE SOLUTION; ON OUTPUT, THE COMPUTED
;           SOLUTION.
...
function tnmin, fcn, xall, fguess=fguess, functargs=fcnargs, parinfo=parinfo, $
  ...
  ;; Parinfo:
  ;; --------------- STARTING/CONFIG INFO (passed in to routine, not
  ;;                 changed)
...
; IN THIS ROUTINE) WITH A FURTHER DIAGONAL SCALING (SEE ROUTINE
;NDIA3).
...
;  IF (NOT LRESET) OR ICYCLE EQ 1 AND n_elements(LSR) GT 0 THEN
;  BEGIN   ;; For testing
  IF (LRESET EQ 0) THEN BEGIN
;
